...
int main() {
	...
	double max_runtime = 45*2; // Max allowable runtime to pass [sec]
	double max_translation_error = 1*2; // Max allowable translation error to pass [m]
 ...
 //cout<<"map_test"<<map.landmark_list[0].id_i<<"\n"<<endl	;
	...
	for (int i = 0; i < num_time_steps; ++i) { //i < num_time_steps
		cout << "\nTime step: " << i << endl;
		...
		if (!pf.initialized()) {
			// get GPS measured value
			n_x = N_x_init(gen);
			n_y = N_y_init(gen);
			n_theta = N_theta_init(gen);
			//Initialize particle filter based on GPS measured values
			//cout<<"Ground truth: x = "<< gt[i].x<<"m, y = "<<gt[i].y<<"m, orientation = " <<gt[i].theta<<" rad/s."<<std::endl;
			pf.init(gt[i].x + n_x, gt[i].y + n_y, gt[i].theta + n_theta, sigma_pos);
		}
		else {
			// Predict the vehicle's next state (noiseless).
			/*cout<<"Control variables: velocity "<<position_meas[i-1].velocity
			    <<" m/s, yaw rate=" << position_meas[i-1].yawrate << "r/s" << endl;*/

			pf.prediction(delta_t, sigma_pos, position_meas[i-1].velocity, position_meas[i-1].yawrate);

			cout<<"ground truth, x="<<gt[i].x<<", y="<<gt[i].y<<", theta = "<<gt[i].theta << endl;
		}
		...
		cout<< "best particle, x =" << best_particle.x << "m, y=" << best_particle.y
							<<"m, orientation="<<best_particle.theta <<" r/s"<<endl;
